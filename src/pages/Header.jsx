...
class Header extends React.Component {
  ...
  getLogoClass = (collapsed, { logo, dark, darkSmall }) =>
    `${logo} ${!collapsed ? dark : darkSmall}`;
  ...
  tebAccount = () => {
    ajax.get('/account/logout').then(data => {
      // storage.clear();
      const loginUrl = data.data.url.replace(
        '%2FPLACEHOLDER%2F',
    ...
    return (
      <Layout.Header className={styles.header}>
        <h1 className={this.getLogoClass(collapsed, styles)}>
          <NavLink to="/">SaaS 商户端</NavLink>
        </h1>
        <div className={styles.storeElement}>欢迎您，North</div>
        <div className={styles.info}>
          ...
          >
            <div className={styles.download}>
              <span className={styles.downloadDot} style={{ display: 'inline-block' }} />
            </div>
          </Popover>
          <Popover
